...
const SocialPostWriterTool: React.FC<SocialPostWriterToolProps> = ({ onBackToDashboard, initialPostType }) => {
    ...
    const renderOutput = () => {
        if (isGenerating) {
            return (
                <div className="text-center">
                    <Spinner />
                    <p className="mt-4 text-cyan-500 dark:text-cyan-400">Crafting your social post...</p>
                </div>
            );
        }
        ...
        if (generatedContent) {
            return (
                <div className="prose dark:prose-invert prose-sm max-w-none whitespace-pre-wrap w-full">{generatedContent}</div>
            );
        }
            ...
            <div className="w-full max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="flex flex-col space-y-6 p-6 bg-white dark:bg-[#161B22] border border-gray-200 dark:border-gray-800 rounded-2xl shadow-lg">
                        {/* Input Type */}
                        <div>
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-200 mb-3"><span className="text-cyan-500 dark:text-cyan-400">1.</span> Provide Context</h2>
                            <div className="flex space-x-2 rounded-lg bg-gray-200 dark:bg-gray-800 p-1">
                                <button onClick={() => setInputType('topic')} className={`w-full py-2 text-sm font-medium rounded-md transition-colors ${inputType === 'topic' ? 'bg-cyan-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-700'}`}>Subject / Topic</button>
                                <button onClick={() => setInputType('url')} className={`w-full py-2 text-sm font-medium rounded-md transition-colors ${inputType === 'url' ? 'bg-cyan-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-700'}`}>URL (Website/Video)</button>
                            </div>
                            {inputType === 'topic' ? (
                                <textarea
                                    value={inputValue}
                                    ...
                                    className="mt-3 w-full h-28 p-3 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-colors"
                                />
                            ) : (
                                <input
                                    type="url"
                                    ...
                                    className="mt-3 w-full p-3 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-colors"
                                />
                            )}
                        </div>
                        {/* Platform */}
                        <div>
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-200 mb-3"><span className="text-cyan-500 dark:text-cyan-400">2.</span> Choose Platform</h2>
                         ...
                         {/* Attributes */}
                        <div>
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-200 mb-3"><span className="text-cyan-500 dark:text-cyan-400">3.</span> Set Attributes</h2>
                            <div className="space-y-4">
                                <div>
                                    <h3 className="font-medium text-gray-600 dark:text-gray-400 mb-2 text-sm">Tone of Voice</h3>
                                    <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                                        {TONE_OPTIONS.map(opt => <OptionButton key={opt} label={opt} isSelected={tone === opt} onClick={() => setTone(opt)} />)}
                                    </div>
                                </div>
                                 <div>
                                    <h3 className="font-medium text-gray-600 dark:text-gray-400 mb-2 text-sm">Length</h3>
                    ...
                    <div className="flex flex-col space-y-4 p-6 bg-white dark:bg-[#161B22] border border-gray-200 dark:border-gray-800 rounded-2xl shadow-lg">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                            <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-200 shrink-0">Generated Content</h2>
                        ...
                        <div className="flex-grow w-full flex flex-col items-stretch justify-start bg-gray-100 dark:bg-black/20 rounded-lg p-4 min-h-[400px] overflow-y-auto space-y-4">
                            {/* Hero Image Section */}
                            {isGeneratingHeroImage && (
                                <div className="text-center">
                                    <Spinner />
                                    <p className="mt-4 text-cyan-500 dark:text-cyan-400">Generating hero image...</p>
                                </div>
                            )}
                            {heroImageError && (
                                <div className="text-center text-red-700 bg-red-100 dark:text-red-400 dark:bg-red-900/50 p-4 rounded-lg">
